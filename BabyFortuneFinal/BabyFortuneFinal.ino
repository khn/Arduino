...
int nFortuneDelay = 6000; // How long the fortune will remain on the OLED.
...
void loop() {
  ...
  if (nIntroTimer < 9000  && nIntroTimer > 6000 && 3 != nIntroDisply) {
  ...
  // If timer has been running for more than 9 seconds and is NOT the fourth intro text option
  if (nIntroTimer > 9000 && 4 != nIntroDisply) {
    // Then set to third text option.
    waiting(4);
    // and set variable to track
    nIntroDisply = 4;
 ...
 nRandomNum = TrueRandom.random(2,29);
  ...
  switch (nRandomNum) { // one plus the number of fortunes here. (Don't forget it starts at 0)
    ...
    case 25:
      ...
      display.println(F("Fatten up your pets.\nYour loved one's\nsurvival will depend\non it."));
      break;
    case 26:
